...
### 🌍 全球市場一覽
- **即時匯率**：監控 TWD 對 USD, EUR, JPY, CNY, CHF, GBP 之匯率（3位精度）。
- **大宗商品**：即時追蹤黃金、白銀、布萊特原油 (Brent) 及德州原油 (WTI)。
...
- **互動式圖表**：使用 Plotly 實現縮放、平移、懸停查看詳細數據。

### 📧 黃金價格 Email 提醒
- **自動監控**：每小時自動檢查黃金價格（台幣/公克）。
- **目標價格設定**：可自訂目標價格（預設 4900 台幣/公克）。
- **Email 通知**：達到目標價格時自動寄送 Email 提醒。
- **背景監控**：使用 GitHub Actions 排程，即使電腦關機也能持續監控。
- **避免重複通知**：同一天只會寄送一次提醒。
...
6. **黃金價格提醒**：開啟側邊欄的「📧 黃金價格 Email 提醒」，設定目標價格，當價格達到時會自動收到 Email 通知。
...
### 背景黃金價格監控 (GitHub Actions)
專案包含 gold_price_monitor.py，可透過 GitHub Actions 每小時自動檢查黃金價格：

設定目標價格：編輯 gold_price_monitor.py 中的 TARGET_PRICE 變數。
設定 Gmail：編輯 SENDER_EMAIL 和 SENDER_PASSWORD（應用程式密碼）。
上傳到 GitHub：推送到 GitHub 後，Actions 會每小時自動執行。
手動觸發：到 GitHub Actions 頁面，點「Run workflow」可立即執行。
